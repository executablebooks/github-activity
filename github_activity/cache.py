...
def _cache_data(query_data, path_cache):
    ...
    for (org, repo), idata in query_data.groupby(['org', 'repo']):
        ...
        if path_pr_cache.exists():
            ...
            prs_cache = prs_cache.append(data_prs, sort=False).drop_duplicates(subset=["url"])
        else:
            prs_cache = data_prs
        ...
        if path_issues_cache.exists():
            ...
            issues_cache = issues_cache.append(data_issues, sort=False).drop_duplicates(subset=["url"])
        else:
        ...
        data_comments = pd.DataFrame(data_comments).rename(columns={"created": "createdAt"})
        ...
        if path_comments_cache.exists():
            ...
            comments_cache = comments_cache.append(data_comments, sort=False).drop_duplicates(subset=["url"])
        else:
...
def load_from_cache(target, kind, path_cache=None):
    ...
    if repo is None:
        repos = [ii for ii in path_cache_org.glob('*') if ii.is_dir() and not ii.name.startswith('.')]
    else:
    ...
    for org in path_cache.glob('*'):
        for repo in org.glob('*'):
            for ipath in repo.glob('*'):
                ...
                out_data.append({'org': org.name, 'repo': repo.name, 'kind': kind, "start": mindate, "end": maxdate, "nrecords": nrecords})
